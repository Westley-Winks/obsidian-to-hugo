...
def get_img_links(text: str) -> List[WikiLink]:
    """
    Get all image links from the given text and return a list of them.
    Each list item is a dictionary with the following keys:
    - wiki_link: the exact match
    - link: the extracted link
    - text: the possible extracted text
...
def img_link_to_hugo_link(wiki_link: WikiLink) -> str:
    """
    Convert the image link into a hugo link.
    """
    # hugo_link = f' [{wiki_link["text"]}]({{{{< ref "{wiki_link["link"]}" >}}}})'
    if wiki_link["text"] == wiki_link["link"]:
        hugo_link = f'![]({wiki_link["link"]})'
    else:
        hugo_link = f'{{{{< image {wiki_link["link"]} Resize "{wiki_link["text"]}x" />}}}}'
...
def replace_wiki_links(text: str) -> str:
    ...
    img_links = get_img_links(text)
    for link in links:
        hugo_link = wiki_link_to_hugo_link(link)
        text = text.replace(link["wiki_link"], hugo_link)
    for img_link in img_links:
        hugo_link = img_link_to_hugo_link(img_link)
        text = text.replace(img_link["wiki_link"], hugo_link)
